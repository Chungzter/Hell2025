...
namespace Unloved::Editor {
    ...
    void UpdateCamera() {
       ...
       //}
        ...
        Unloved::Viewport* viewport = Unloved::ViewportManager::GetViewportByIndex(Editor::GetActiveViewportIndex());
        ...
        if (!Input::KeyDown(HELL_KEY_LEFT_ALT)) {
            for (int i = 0; i < 4; i++) {
                Unloved::Viewport* queryViewport = Unloved::ViewportManager::GetViewportByIndex(i);
