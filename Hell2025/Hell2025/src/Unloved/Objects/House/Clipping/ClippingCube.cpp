...
void ClippingCube::Update(const Hell::Transform& transform) {
...
void ClippingCube::DrawDebugCorners(const glm::vec4& color) {
    for (glm::vec3& corner : m_corners) {
        DebugDraw::DrawPoint(corner, color);
    }
}

void ClippingCube::DrawDebugEdges(const glm::vec4& color) {
    int edges[12][2] = {
        {0, 1}, {0, 2}, {0, 4},
        {1, 3}, {1, 5},
        {2, 3}, {2, 6},
