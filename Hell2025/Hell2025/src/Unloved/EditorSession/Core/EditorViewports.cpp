...
namespace Unloved::EditorSession::Viewports {
    namespace {
        ...
        void BeginOrbitCamera(uint32_t viewportIndex) {
            EditorViewportState& viewport = g_viewports[viewportIndex];
            const bool heightMapMode = Unloved::EditorSession::HasMode() && Unloved::EditorSession::GetMode() == EditorSessionMode::MAP && MapTools::GetMode() == MapTools::Mode::HEIGHT_MAP;
            if (!heightMapMode) {
                viewport.hasCustomOrbitPoint = false;
            }
    ...
    void SetOrbitPoint(uint32_t viewportIndex, const glm::vec3& orbitPoint) {
        if (viewportIndex >= g_viewports.size()) return;
        if (!Unloved::EditorSession::HasMode() || Unloved::EditorSession::GetMode() != EditorSessionMode::MAP || MapTools::GetMode() != MapTools::Mode::HEIGHT_MAP) return;
