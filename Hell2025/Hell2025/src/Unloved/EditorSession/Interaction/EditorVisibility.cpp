...
namespace Unloved::EditorSession::Visibility {
    ...
    bool ShouldHide(uint64_t objectId) {
        ...
        if (GetObjectIdType(objectId) == ObjectType::ANIMATED_GAME_OBJECT) {
            AnimatedGameObject* animatedGameObject = World::GetAnimatedGameObjectByObjectId(objectId);
            return animatedGameObject && g_hiddenObjectIds.find(animatedGameObject->GetOwnerObjectId()) != g_hiddenObjectIds.end();
        }
