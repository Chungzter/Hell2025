...
namespace Unloved::EditorSession::Selection {
    ...
    void SelectPoint(uint64_t objectId, int32_t pointIndex, PointSequences::PointHandleType handleType) {
        ...
        const bool localAxes = GetObjectIdType(objectId) == ObjectType::WORLD_PLANE || GetObjectIdType(objectId) == ObjectType::PLANAR_QUAD_OBJECT || GetObjectIdType(objectId) == ObjectType::POINT_PAIR_OBJECT;
