...
namespace Unloved::EditorSession::HeightMapEditor {
    namespace {
        ...
        float GetBrushDirection(bool leftMouseDown, bool rightMouseDown) {
            if (MapTools::GetHeightMapTool() != MapTools::HeightMapTool::ADD) return 0.0f;
    ...
    void Update(bool allowMouseInput) {
        ...
        if (Workspace::HasMode() && Workspace::GetMode() == EditorSessionMode::MAP && MapTools::GetMode() == MapTools::Mode::HEIGHT_MAP) {
            LoadBrush();
        }
        ...
        const MapTools::HeightMapTool tool = MapTools::GetHeightMapTool();
        ...
        if (hit.found && tool == MapTools::HeightMapTool::FLAT && controlDown && Hell::Input::LeftMousePressed()) {
        ...
        if (hit.found && tool == MapTools::HeightMapTool::ADD) {
        ...
        if (hit.found && tool == MapTools::HeightMapTool::FLAT && leftMouseDown && brushEvent) {
            heightPainted = PaintTargetHeight(*mapData, hit.position);
        }
        if (hit.found && HasValidTerrainControlData(*mapData) && tool == MapTools::HeightMapTool::TEXTURE_PAINT && leftMouseDown) {
            terrainControlPaintBounds = PaintTexture(*mapData, hit.position);
        }
        if (hit.found && tool == MapTools::HeightMapTool::SMOOTH && leftMouseDown && brushEvent) {
            heightPainted = PaintSmooth(*mapData, hit.position);
        }
        if (hit.found && HasValidTerrainControlData(*mapData) && tool == MapTools::HeightMapTool::TEXTURE_SPRAY && leftMouseDown && brushEvent) {
            terrainControlPaintBounds = SprayTexture(*mapData, hit.position);
        }
        if (hit.found && HasValidTerrainControlData(*mapData) && tool == MapTools::HeightMapTool::AUTO_SHADER && (leftMouseDown || rightMouseDown)) {
