...
namespace Unloved::EditorSession::Viewports {
    namespace {
        ...
        const char* GetModeLabel(uint32_t viewportIndex) {
            if (g_flyModeViewportIndex == static_cast<int32_t>(viewportIndex)) return "Perspective (Fly mode)";
        ...
        void UpdateFlyLook(EditorViewportState& viewport) {
            ...
            if ((currentPitch >= FREE_LOOK_TOP_LIMIT && pitchDelta > 0.0f) || (currentPitch <= FREE_LOOK_BOTTOM_LIMIT && pitchDelta < 0.0f)) pitchDelta = 0.0f;
            else pitchDelta = std::clamp(currentPitch + pitchDelta, FREE_LOOK_BOTTOM_LIMIT, FREE_LOOK_TOP_LIMIT) - currentPitch;
        ...
        void UpdateFlyMovement(EditorViewportState& viewport) {
            ...
            if (Hell::Input::KeyDown(HELL_KEY_W)) movement += viewport.camera.GetForward();
            if (Hell::Input::KeyDown(HELL_KEY_S)) movement -= viewport.camera.GetForward();
            if (Hell::Input::KeyDown(HELL_KEY_A)) movement -= viewport.camera.GetRight();
            if (Hell::Input::KeyDown(HELL_KEY_D)) movement += viewport.camera.GetRight();
            if (Hell::Input::KeyDown(HELL_KEY_Q)) movement += EDITOR_WORLD_UP;
            if (Hell::Input::KeyDown(HELL_KEY_E)) movement -= EDITOR_WORLD_UP;
        ...
        void ZoomViewport(uint32_t viewportIndex, bool zoomIn) {
            ...
            if (Hell::Input::KeyDown(HELL_KEY_LEFT_SHIFT_GLFW)) stepStrength *= 0.5f;
            else if (Hell::Input::KeyDown(HELL_KEY_LEFT_CONTROL_GLFW)) stepStrength *= 2.5f;
    ...
    void PrepareInitialView(const CreateInfoCollection& createInfoCollection) {
        ...
        for (const WallCreateInfo& wall : createInfoCollection.walls) {
            for (const glm::vec3& point : wall.points) {
                growBounds(point);
                // Wall points sit on the floor so include the top edge too
                growBounds(point + glm::vec3(0.0f, wall.height, 0.0f));
            }
        }
    ...
    void ApplyInitialView() {
        // Consume this once after the map layout is ready
        ...
        // Start above the scene with positive X pointing away from the camera
        g_viewports[0].camera.LookAt(center, glm::normalize(glm::vec3(1.0f, -0.75f, -1.0f)), EDITOR_WORLD_UP);
    ...
    void Update() {
        ...
        for (uint32_t i = 0; i < g_viewports.size(); i++) {
            ...
            if (region->rect.Contains(mousePosition)) g_hoveredViewportIndex = static_cast<int32_t>(i);
        }
    }
    ...
    void UpdateInput(bool allowKeyboardInput, bool allowMouseInput) {
        ...
        if (allowKeyboardInput && g_hoveredViewportIndex >= 0) {
            ...
            else if (Hell::Input::KeyPressed(HELL_KEY_NUMPAD_1)) SetMode(viewportIndex, EditorViewportMode::FRONT);
            else if (Hell::Input::KeyPressed(HELL_KEY_NUMPAD_3)) SetMode(viewportIndex, EditorViewportMode::BACK);
            else if (Hell::Input::KeyPressed(HELL_KEY_NUMPAD_4)) SetMode(viewportIndex, EditorViewportMode::LEFT);
            else if (Hell::Input::KeyPressed(HELL_KEY_NUMPAD_6)) SetMode(viewportIndex, EditorViewportMode::RIGHT);
            else if (Hell::Input::KeyPressed(HELL_KEY_NUMPAD_5)) SetMode(viewportIndex, EditorViewportMode::TOP);
            else if (Hell::Input::KeyPressed(HELL_KEY_NUMPAD_2)) SetMode(viewportIndex, EditorViewportMode::BOTTOM);
            else if (Hell::Input::KeyPressed(HELL_KEY_C))        BeginPivotTransition(g_viewports[viewportIndex]);
        }
        ...
        if (g_cameraDragViewportIndex >= 0 && Hell::Input::MiddleMouseDown()) {
            ...
            if (Hell::Input::KeyDown(HELL_KEY_LEFT_SHIFT_GLFW)) {
                ...
                if (!viewport.panActive) BeginPanCamera(viewportIndex);
                if (viewport.panActive)  PanCamera(viewportIndex);
            }
            else {
                viewport.panActive = false;

                if (!viewport.orbitActive) BeginOrbitCamera(viewportIndex);
    ...
    void RenderLabels() {
        constexpr const char* WHITE_TEXT_COLOR_TAG = "[COL=1.0,1.0,1.0,1.0]";
        constexpr int32_t LABEL_PADDING = 8;

        for (uint32_t i = 0; i < g_viewports.size(); i++) {
            const EditorViewportRegion* region = Layout::GetViewportRegionByIndex(i);
            if (!region || !region->visible) continue;

            const glm::ivec2 position(region->rect.x + LABEL_PADDING, region->rect.y + LABEL_PADDING);
            UIBackEnd::BlitText(UICanvas::NATIVE, std::string(WHITE_TEXT_COLOR_TAG) + GetModeLabel(i), Style::FONT_NAME, position, Alignment::TOP_LEFT, Style::FONT_SCALE, TextureFilter::NEAREST);
        }
    }
