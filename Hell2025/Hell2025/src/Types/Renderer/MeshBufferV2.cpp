#include "MeshBufferV2.h"
#include <algorithm>
#include <Hell/Logging.h>
#include <Util/Util.h>

void MeshBufferV2::Initilize() {
    Reset();
    InitOpenGL();
    m_initilized = true;
}

void MeshBufferV2::Reset() {
    m_meshes.clear();
    m_vertices.clear();
    m_indices.clear();
...
uint64_t MeshBufferV2::AddMesh(const std::vector<Vertex>& vertices, const std::vector<uint32_t>& indices, const std::string& name) {
...
int32_t MeshBufferV2::AddVertices(const std::vector<Vertex>& newVertices) {
...
int32_t MeshBufferV2::AddIndices(const std::vector<uint32_t>& newIndices) {
    int32_t freeMemoryBlockIndex = -1;
    int32_t insertOffset = 0;
...
int32_t MeshBufferV2::AllocateExtraVertexSpace(size_t vertexCount) {
...
int32_t MeshBufferV2::AllocateExtraIndexSpace(size_t indexCount) {
...
void MeshBufferV2::RemoveMesh(uint64_t meshIndex) {
...
void MeshBufferV2::PreAllocate(size_t maxVertices, size_t maxIndices) {
...
Mesh* MeshBufferV2::GetMeshById(uint64_t meshId) {
...
std::span<Vertex> MeshBufferV2::GetMeshVertexSpan(uint64_t meshId) {
...
std::span<uint32_t> MeshBufferV2::GetMeshIndexSpan(uint64_t meshId) {
    Mesh* mesh = GetMeshById(meshId);
    if (!mesh) return {};
...
void MeshBufferV2::PrintDebugInfo() {
...
void MeshBufferV2::InsertVerticesToOpenGLVertexBuffer(const std::vector<Vertex>& vertices, uint32_t insertOffset) {
...
void MeshBufferV2::InsertIndicesToOpenGLIndexBuffer(const std::vector<uint32_t>& indices, uint32_t insertOffset) {
...
void MeshBufferV2::ResizeOpenGLVertexBuffer(size_t totalCount) {
...
void MeshBufferV2::ResizeOpenGLIndexBuffer(size_t totalCount) {
...
void MeshBufferV2::PreAllocateOpenGL(size_t maxVertices, size_t maxIndices) {
    if (m_vbo != 0) glDeleteBuffers(1, &m_vbo);
    if (m_ebo != 0) glDeleteBuffers(1, &m_ebo);
