    ...
    namespace {
        ...
        bool ValidateTexture(Texture& texture) {
            ...
            if (imageData.mips.empty()) {
                Logging::Error() << "OpenGLTextureUploader::ValidateTexture(..) failed because texture '" << texture.GetFileName() << "' has no image data\n";
                return false;
            }

            if (OpenGLUtil::ImageFormatToGLFormat(imageData.format) == GL_NONE || OpenGLUtil::ImageFormatToGLInternalFormat(imageData.format) == GL_NONE) {
                Logging::Error() << "OpenGLTextureUploader::ValidateTexture(..) failed because texture '" << texture.GetFileName() << "' has an unsupported image format\n";
                return false;
            }
            ...
            for (size_t mipIndex = 0; mipIndex < uploadMipCount; ++mipIndex) {
                ...
                if (mip.width == 0 || mip.height == 0 || mip.data.empty()) {
                    Logging::Error() << "OpenGLTextureUploader::ValidateTexture(..) failed because texture '" << texture.GetFileName() << "' has invalid mip " << mipIndex << "\n";
                    return false;
                }

                if (mip.data.size() > static_cast<size_t>(std::numeric_limits<GLsizei>::max())) {
                    Logging::Error() << "OpenGLTextureUploader::ValidateTexture(..) failed because mip " << mipIndex << " of texture '" << texture.GetFileName() << "' is too large\n";
                    return false;
                }
            }
