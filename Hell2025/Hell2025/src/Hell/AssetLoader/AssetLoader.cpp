...
namespace Hell::AssetLoader {
    ...
    namespace {
        ...
        struct AnimationLoadJob {
            ...
            FileInfo fileInfo;
            std::future<AnimationData> future;
            bool awaitingWork = true;
        };
        ...
        void CreateAnimationLoadJob(Animation& animation) {
            AnimationLoadJob& animationLoadJob = g_animationLoadJobs.emplace_back();
            animationLoadJob.animationName = animation.GetName();
            animationLoadJob.fileInfo = animation.GetFileInfo();
        }
        ...
        void PollAnimationJobs() {
            ...
            for (size_t i = 0; i < g_animationLoadJobs.size();) {
                ...
                if (job.awaitingWork) {
                    ...
                    if (Animation* animation = ResourceManager::GetAnimationPtr(job.animationName)) {
                        animation->SetLoadState(LoadState::LOADING);
                    }

                    job.awaitingWork = false;
                    job.future = std::async(std::launch::async, LoadAnimationData, job.fileInfo);
                    activeJobCount++;
                    i++;
                    continue;
                }
                ...
                if (job.future.wait_for(std::chrono::seconds::zero()) == std::future_status::ready) {
                    AnimationData result = job.future.get();
                    const bool loaded = result.animation.GetLoadState() == LoadState::LOADED;

                    Animation* animation = ResourceManager::GetAnimationPtr(job.animationName);

                    if (animation) {
                        *animation = std::move(result.animation);
    ...
    void PreAllocateAssetGeometry() {
        ...
        meshBuffer.PreAllocate(vertexCount, indexCount, vertexWeightCount);
    }
    ...
    uint32_t CreateSkinnedMesh(const SkinnedMeshData& skinnedMeshData) {
        ...
        const uint32_t meshId = meshBuffer.AddSkinnedMesh(skinnedMeshData.vertices, skinnedMeshData.indices, skinnedMeshData.vertexWeights, metadata, skinnedMeshData.name);
        Mesh* mesh = meshBuffer.GetMeshById(meshId);
