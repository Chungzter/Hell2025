...
namespace Hell::File {
    ...
    bool RenameFile(const std::string& oldFilePath, const std::string& newFilePath) {
        if (!FileExists(oldFilePath)) {
            Logging::Error() << "Util::RenameFile() failed because old path '" << oldFilePath << "' does not exist!\n";
            return false;
        }
        ...
        try {
            ...
            Logging::Error() << "Util::RenameFile() failed: " << e.what() << "\n";
            return false;
        }
    }
    ...
    FileInfo GetFileInfoFromPath(const std::string& filepath) {
        ...
        if (!FileExists(filepath)) {
            Logging::Error() << "Util::GetFileInfoFromPath() Failed: path does not exist: " << filepath << ", warning returning an empty FileInfo\n";
            return fileInfo;
        }
